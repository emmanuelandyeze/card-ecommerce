...
export default function MenuItemTile({onAddToCart, ...item}) {
  ...
  return (
      ...
      group hover:bg-white hover:shadow-md hover:rounded-lg hover:shadow-black/25 transition-all"
		>
			...
			<h4 className="font-semibold text-md text-left my-1">
				{name}
			</h4>
			<p className="text-gray-500 text-sm text-left line-clamp-3 hidden md:block">
				{description}
			</p>
			<p className="text-left">${basePrice}</p>
			<AddToCartButton
				image={image}
